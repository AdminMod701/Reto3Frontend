...
function registrar() {
    ...
    if (validar()){
        $.ajax({
            ...
            url: "http://140.238.191.252:8080/api/Costume/save",
            ...
            error: function (xhr, status) {
                $("#mensajes").show(1000);
                $("#mensajes").html("Error peticion POST...ooooooo" + status );
                //$("#mensajes").hide(1000);
            }
        });
    }
...
function listarCategorias() {
    $.ajax({
        // la URL para la petición (url: "url al recurso o endpoint")
        url: "http://140.238.191.252:8080/api/Category/all",
        ...
        complete: function (xhr, status) {
            $("#mensajes").html("Obteniendo listado de disfrases...");
            $("#mensajes").hide(1000);
        }
    });
}